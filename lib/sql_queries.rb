...
"SELECT projects.title, ABS(projects.funding_goal - SUM(pledges.amount)) FROM projects JOIN pledges  ON projects.id = pledges.project_id GROUP BY projects.title HAVING SUM(pledges.amount) >= projects.funding_goal;"
end
